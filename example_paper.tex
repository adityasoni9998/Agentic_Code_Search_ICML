...
\twocolumn[
  \icmltitle{Code-Search OSS: \\ Training Open-Source LLM Agents with Reinforcement Learning for Repository-Level Localization}
...
\begin{abstract}
A prerequisite for resolving issues in large codebases is code localization—the identification of relevant files, functions, and lines of code that must be modified. Effective localization requires navigating complex software repositories and rapidly acquiring contextual understanding. Traditionally, automated software engineering systems rely on a single agent to perform both code localization and code editing. However, recent work in the agent building community has shown that we can train specialized and highly efficient subagents to perform the code localization subroutine. Despite this promise, most existing approaches remain closed-source. In this work, we present an open-source recipe for training a dedicated code localization agent. We re-purpose established issue-resolution benchmarks (SWE-Smith and SWE-Bench) for localization tasks and integrate them with the OpenHands agent harness. Using this framework, we train \texttt{CS-OSS} (Code-Search OSS), a specialized code localization agent that achieves competitive performance across SWE-Bench Lite, Verified, and Pro.
\end{abstract}
