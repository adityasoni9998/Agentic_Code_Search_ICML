...
On repository-level coding tasks such as those in the popular SWE-Bench benchmark \citep{jimenez2024swebenchlanguagemodelsresolve}, a critical first step is \textbf{code localization}: given an issue description and a repository, the system must identify the relevant files and finer-grained code entities (e.g., classes and functions) to modify~(\autoref{sec:background}; \citet{husain2019codesearchnet,xia2024agentless}).
This is challenging in large repositories with complex interdependencies, and relying on general-purpose language models to solve this localization problem as part of the agentic coding loop can result in high costs, failed fixes, and code bloat~\citep{hong2025context}.
...
Traditionally, this code search was performed through semantic code search using vector databases \citep{xia2024agentless,wang2025coderag,xie2025swe}.

In contrast, a number of recent methods have investigated \emph{agentic code search}, using agents to iteratively uncover the evidence that is necessary to solve the issue under consideration.
Typically these methods have involved significant modifications to the agent itself to incorporate static analysis of the codebase, such as LocAgent's repository graph navigation \citep{chen-etal-2025-locagent} and RepoNavigator's ``jump'' tool, allowing jumping between files \citep{zhang2025one}.
While well-grounded, these necessitate the use of static analysis tools tailored to a particular programming language, increasing operational complexity of deploying such agents on a wide variety of coding scenarios.
Simultaneously, there have been a number of anecdotal reports from industry regarding reinforcement learning methods that increase the ability of agents to perform localization rapidly \citep{cognition2025swegrep,cursor2025composer}, with varying levels of detail but with no clear training recipe.

Given this background, we address the fundamental research question: \emph{given an appropriate reinforcement learning algorithm, what is an effective recipe to train a code localization agent that simply uses terminal and file-use tools typical of more generic coding agents, yet achieves competitive or superior accuracy with agents using special-purpose tools?}
In this work, we provide the first public demonstration of such a recipe, which involves scalable methods for data and environment creation (\autoref{sec:data_creation}), a standard agent scaffold (\autoref{sec:agent_scaffold}), as well as careful attention to reward design (\autoref{sec:reward_design}) and training methods (\autoref{sec:train_algo}).
In experiments (\autoref{sec:experiments}, \autoref{sec:results}), we find that the resulting model \methodname achieves accuracy greater than or competitive with existing methods on a variety of localization datasets derived from SWE-Bench Lite, Verified, and Pro.
Importantly, all models and code will be released publicly, building a strong baseline for future development in the community to build upon.
