\section{\methodname: Our Effective RL Recipe for Code Search}
...
Using this patch-processing pipeline, we curate training instances from GitHub issues collected by prior work~\citep{yang2025swesmithscalingdatasoftware, pan2025trainingsoftwareengineeringagents}, originally intended for training agents to fix issues. Note that we discard issues whose PRs create or delete files, as the agent cannot predict the exact filename of newly created files, and it is not possible to determine the ground-truth modules or functions for deleted files. We further ignore non-Python files (e.g., \texttt{README.md}) modified by the PR from the ground truth, since function- and module-level information cannot be extracted from them. Finally, we remove instances with empty issue descriptions.

To construct the RL environment for our agent, we simply need to the clone the pre-PR commit of repository $\mathcal{R}$ into a fixed path known to the agent through its prompt. As the localization task does not require executing repository code, we do not require installing project dependencies or use sandboxing/containerization. Notably, as our agent scaffold only requires a bash terminal, the overhead of setting up the environment is \emph{significantly} lower compared to prior approaches whose agents require tools that leverage code graphs, vector databases, or dependency parsers. \sonicomment{TODO: For each complex tool, cite the paper that used this tool}

\subsection{Agent Scaffold}\label{sec:agent_scaffold}

We utilize the OpenHands Software Agent SDK~\cite{wang2025openhandssoftwareagentsdk} as the agent scaffold for \methodname, due to its strong performance on a broad range of software engineering benchmarks, its robust implementation of essential tools (e.g., the terminal), in-built support for parallel tool-calling, and its modular design that facilitates seamless integration with our training backend. 

The agent is primarily equipped with the \texttt{Terminal} tool that supports execution of standard Unix commands like \texttt{rg} (ripgrep), \texttt{find}, \texttt{ls}, \texttt{grep}, \texttt{sed}, etc. To accelerate grep-style code search, we install ripgrep~\footnote{\url{https://github.com/BurntSushi/ripgrep}} in the agent's environment and explicitly prompt the agent to use ripgrep. In addition, the agent is provided with a \texttt{LocalizationFinish} tool to submit its predicted files, modules, and functions.

Our initial experiments adopted the string-based output format from LocAgent~\citep{chen-etal-2025-locagent}, but we found it brittle and sensitive to noisy reward signals due to limited format validation. We address this by requiring the agent to terminate via the \texttt{LocalizationFinish} tool, which enforces a structured output schema and enables precise parsing and validation, thereby improving the fidelity of the reward signal. Detailed system and user prompts used in our scaffold are provided in Appendix~\ref{appendix:prompts}.
...
We also explored additional auxiliary rewards, such as explicitly incentivizing parallel tool calling motivated by SWE-grep~\citep{cognition2025swegrep}, but found that these interventions degraded overall performance. Thus, we simply fallback to explicitly prompting the agent to use parallel tool-calling (Appendix \ref{appendix:prompts}).

\subsection{RL Training Algorithm}\label{sec:train_algo}
We implement our training backend using SkyRL~\cite{griggs2025skrylv01}, a modular framework for reinforcement learning with language models that provides key capabilities such as asynchronous training~\citep{fu2025areallargescaleasynchronousreinforcement}. Asynchronous training improves GPU utilization by parallelizing rollout generation and weight optimization. Specifically, parameter updates are triggered once a sufficient number of rollouts are collected, allowing trajectories sampled from slightly stale model checkpoints. Concretely, trajectories used during the $n^{\text{th}}$ training step may be sampled from model checkpoints that are at most $t$ steps stale, where $t$ denotes the maximum staleness (set to 4 in our experiments). After each optimization step, the updated weights are synchronized with the vLLM~\citep{kwon2023efficient} inference engine, and any in-progress inference requests are terminated to ensure consistency.
